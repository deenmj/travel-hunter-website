...
export default function EditDestinationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchDestination = async () => {
      ...
      if (result.data) {
        const d = result.data as Destination
        setForm({
          name: d.name || '',
          slug: d.slug || '',
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Update the details of this destination.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700/50 p-6 space-y-4">
          <h2 className="text-base font-semibold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3">Highlights</h2>
          <div className="flex gap-2">
            ...
            />
            <button type="button" onClick={addHighlight} className="px-4 h-12 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
          ...
          {form.highlights.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {form.highlights.map((h, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-lg text-sm">
                  {h}
                  <button type="button" onClick={() => removeHighlight(i)} className="hover:text-red-500 transition-colors">
        ...
        <div className="flex items-center gap-3 justify-end pt-2">
          <Link href={ROUTES.ADMIN_DESTINATIONS}>
            <button type="button" className="px-5 py-2.5 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Cancel</button>
          </Link>
          <button
            type="submit"
          ...
          >
            {saving ? (
              ...
              <><Save className="w-4 h-4" />Update Destination</>
            )}
          </button>
        </div>
      </form>
